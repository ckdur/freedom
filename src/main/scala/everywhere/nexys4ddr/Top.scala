...
package sifive.freedom.everywhere.nexys4ddr
...
trait Nexys4DDRPeripheryConfigs {
...
// This custom Nexys4DDR coreplex has no port into the L2 and no memory subsystem

class Nexys4DDRCoreplex(implicit p: Parameters) extends BareCoreplex
    with CoreplexNetwork
    with CoreplexRISCVPlatform
    with RocketTiles {
  override lazy val module = new Nexys4DDRCoreplexModule(this, () => new Nexys4DDRCoreplexBundle(this))
}

class Nexys4DDRCoreplexBundle[+L <: Nexys4DDRCoreplex](_outer: L) extends BareCoreplexBundle(_outer)
    with CoreplexNetworkBundle
    with CoreplexRISCVPlatformBundle
    with RocketTilesBundle

class Nexys4DDRCoreplexModule[+L <: Nexys4DDRCoreplex, +B <: Nexys4DDRCoreplexBundle[L]](_outer: L, _io: () => B)
  extends BareCoreplexModule(_outer, _io)
    with CoreplexNetworkModule
    with CoreplexRISCVPlatformModule
    with RocketTilesModule

class Nexys4DDRSystem(implicit p: Parameters) extends BaseTop
    with Nexys4DDRPeripheryConfigs
  ...
  override lazy val module = new Nexys4DDRSystemModule(this, () => new Nexys4DDRSystemBundle(this))

  val coreplex = LazyModule(new Nexys4DDRCoreplex)
  socBus.node := coreplex.mmio
  coreplex.mmioInt := intBus.intnode
}

class Nexys4DDRSystemBundle[+L <: Nexys4DDRSystem](_outer: L) extends BaseTopBundle(_outer)
    with Nexys4DDRPeripheryConfigs
...
class Nexys4DDRSystemModule[+L <: Nexys4DDRSystem, +B <: Nexys4DDRSystemBundle[L]](_outer: L, _io: () => B)
...
class Nexys4DDRTopIO(implicit val p: Parameters) extends Bundle with Nexys4DDRPeripheryConfigs {
  val pads = new Bundle {
...
class Nexys4DDRTop(implicit val p: Parameters) extends Module with Nexys4DDRPeripheryConfigs {
  val sys = Module(LazyModule(new Nexys4DDRSystem).module)
  val io = new Nexys4DDRTopIO
